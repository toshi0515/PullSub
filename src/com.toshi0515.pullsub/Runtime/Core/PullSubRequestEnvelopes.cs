...
namespace PullSub.Core
{
    ...
    public enum PullSubResponseEnvelopeStatus
    {
        Success = 0,
        RemoteError = 1,
    }

    public struct PullSubRequestEnvelope<TRequest>
    {
    ...
    public struct PullSubResponseEnvelope<TResponse>
    {
        public string CorrelationId { get; set; }
        public PullSubResponseEnvelopeStatus Status { get; set; }
        public string ErrorMessage { get; set; }
        public DateTime RespondedUtc { get; set; }
        public TResponse Response { get; set; }
    }

    public sealed class PullSubRequestEnvelopeCodec<TRequest> : IPayloadCodec<PullSubRequestEnvelope<TRequest>>
    {
        ...
        public static readonly PullSubRequestEnvelopeCodec<TRequest> Default
            = new PullSubRequestEnvelopeCodec<TRequest>();

        private readonly JsonSerializerOptions _options;
        private readonly IPayloadCodec<TRequest> _requestCodec;

        public PullSubRequestEnvelopeCodec(
            IPayloadCodec<TRequest> requestCodec = null,
            JsonSerializerOptions options = null)
        {
            _requestCodec = requestCodec ?? PullSubJsonPayloadCodec<TRequest>.Default;
            _options = options ?? DefaultOptions;
        }

        public override bool Equals(object obj)
        {
            return obj is PullSubRequestEnvelopeCodec<TRequest> other
        ...
        public void Encode(
            ...
            PullSubRequestEnvelope<TRequest> value,
            IBufferWriter<byte> bufferWriter)
        {
            ...
            var normalized = PullSubTimestampUtility.NormalizeOrNow(timestampUtc);
            var envelope = value;
        ...
        public bool TryDecode(
            ...
            out PullSubRequestEnvelope<TRequest> value,
            ...
            out string error)
        {
            ...
            try
            {
                ...
                timestampUtc = sentUtc == default
                    ? PullSubTimestampUtility.NormalizeOrNow(requestTimestampUtc)
                    : PullSubTimestampUtility.NormalizeOrNow(sentUtc);

                if (sentUtc == default)
                    sentUtc = timestampUtc;

                value = new PullSubRequestEnvelope<TRequest>
                {
        ...
        private bool UseInlineJsonRequest
            => _requestCodec is PullSubJsonPayloadCodec<TRequest>
            || _requestCodec is PullSubFlatJsonPayloadCodec<TRequest>;
    ...
    public sealed class PullSubResponseEnvelopeCodec<TResponse> : IPayloadCodec<PullSubResponseEnvelope<TResponse>>
    {
        private static readonly JsonSerializerOptions DefaultOptions = new JsonSerializerOptions
        {
            PropertyNameCaseInsensitive = true,
        ...
        public static readonly PullSubResponseEnvelopeCodec<TResponse> Default
            = new PullSubResponseEnvelopeCodec<TResponse>();

        private readonly JsonSerializerOptions _options;
        private readonly IPayloadCodec<TResponse> _responseCodec;

        public PullSubResponseEnvelopeCodec(
            IPayloadCodec<TResponse> responseCodec = null,
            JsonSerializerOptions options = null)
        {
            _responseCodec = responseCodec ?? PullSubJsonPayloadCodec<TResponse>.Default;
            _options = options ?? DefaultOptions;
        }

        public override bool Equals(object obj)
        {
            return obj is PullSubResponseEnvelopeCodec<TResponse> other
                && ReferenceEquals(_options, other._options)
        ...
        public void Encode(
            DateTime timestampUtc,
            PullSubResponseEnvelope<TResponse> value,
            IBufferWriter<byte> bufferWriter)
        {
            if (bufferWriter == null)
                throw new ArgumentNullException(nameof(bufferWriter));
        ...
        public bool TryDecode(
            ReadOnlySpan<byte> payload,
            out PullSubResponseEnvelope<TResponse> value,
            out DateTime timestampUtc,
            out string error)
        {
            value = default;
            ...
            try
            {
                ...
                timestampUtc = respondedUtc == default
                    ? PullSubTimestampUtility.NormalizeOrNow(responseTimestampUtc)
                    : PullSubTimestampUtility.NormalizeOrNow(respondedUtc);

                if (respondedUtc == default)
                    respondedUtc = timestampUtc;

                value = new PullSubResponseEnvelope<TResponse>
                {
                    CorrelationId = correlationId,
                    Status = status,
        ...
        private bool UseInlineJsonResponse
            => _responseCodec is PullSubJsonPayloadCodec<TResponse>
            || _responseCodec is PullSubFlatJsonPayloadCodec<TResponse>;
        ...
        private static bool TryReadStatus(JsonElement root, out PullSubResponseEnvelopeStatus status)
        {
            ...
            if (element.ValueKind == JsonValueKind.Number)
            {
                if (element.TryGetInt32(out var intValue)
                    && Enum.IsDefined(typeof(PullSubResponseEnvelopeStatus), intValue))
                {
                    status = (PullSubResponseEnvelopeStatus)intValue;
                    return true;
                }
    ...
    internal sealed class PullSubRawBinaryPayloadCodec : IPayloadCodec<byte[]>
    {
        public static readonly PullSubRawBinaryPayloadCodec Default = new PullSubRawBinaryPayloadCodec();
