...
{
    internal sealed class ReplyInboxLease : IAsyncDisposable
    {
        private readonly PullSubRuntime _runtime;
        private readonly ITopic<byte[]> _replyTopic;
        private readonly PendingRequestStore _pendingStore;
        private readonly TimeSpan _idleTimeout;
        private readonly QueueOptions _replyQueueOptions;
        ...
        private ValueTask HandleReplyAsync(byte[] responseEnvelopePayload, CancellationToken cancellationToken)
        {
            ...
            if (!ResponseEnvelopeCodec<byte[]>.TryReadCorrelationId(responseEnvelopePayload, out var correlationId, out _))
                return default;
        ...
        private void ThrowIfDisposed()
        {
            if (Volatile.Read(ref _disposed) != 0)
                throw new ObjectDisposedException(nameof(ReplyInboxLease));
        }
    }
}
