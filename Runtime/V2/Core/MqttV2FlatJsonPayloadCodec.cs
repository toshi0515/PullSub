...
namespace UnityMqtt.V2.Core
{
    ...
    /// </summary>
    public sealed class MqttV2FlatJsonPayloadCodec<T> : MqttV2TextPayloadCodec<T>
    {
        ...
        public static readonly MqttV2FlatJsonPayloadCodec<T> Default = new MqttV2FlatJsonPayloadCodec<T>();

        private readonly JsonSerializerOptions _options;

        public MqttV2FlatJsonPayloadCodec(JsonSerializerOptions options = null)
        {
        ...
        {
            return obj is MqttV2FlatJsonPayloadCodec<T> other
        ...
        public override byte[] Encode(DateTime timestampUtc, T value)
        {
            ...
            using (var writer = new Utf8JsonWriter(buffer))
            {
                ...
                if (tDoc.RootElement.ValueKind != JsonValueKind.Object)
                    throw new InvalidOperationException("MqttV2FlatJsonPayloadCodec<T> requires T to serialize as a JSON object.");
                ...
                foreach (var prop in tDoc.RootElement.EnumerateObject())
                {
                    if (prop.Name.Equals("timestamp", StringComparison.Ordinal))
                    {
                        throw new InvalidOperationException(
                            "Property name 'timestamp' is reserved by MqttV2FlatJsonPayloadCodec<T>. Use a different property name.");
                    }
