...
{
    public sealed class MqttV2RawMessage
    {
        public MqttV2RawMessage(string topic, byte[] payload, DateTime receivedUtc)
        {
            Topic = topic;
            Payload = payload;
    ...
    internal sealed class MqttV2RawInbox
    {
        // TryDequeue and DequeueAsync can be mixed on the same topic.
        // Wakeups are not fair between sync and async consumers.
        private readonly object _gate = new object();
        private readonly Dictionary<string, Queue<MqttV2RawMessage>> _queues
            = new Dictionary<string, Queue<MqttV2RawMessage>>(StringComparer.Ordinal);
        private readonly Dictionary<string, MqttV2RawSubscriptionOptions> _optionsByTopic
            = new Dictionary<string, MqttV2RawSubscriptionOptions>(StringComparer.Ordinal);
        ...
        public void ConfigureTopic(string topic, MqttV2RawSubscriptionOptions options)
        {
            ...
            lock (_gate)
            {
                ...
                if (!_queues.ContainsKey(topic))
                    _queues[topic] = new Queue<MqttV2RawMessage>();
        ...
        public void Enqueue(string topic, byte[] payload)
        {
            ...
            lock (_gate)
            {
                ...
                {
                    queue = new Queue<MqttV2RawMessage>();
                    _queues[topic] = queue;
                }
                ...
                queue.Enqueue(new MqttV2RawMessage(topic, payload, DateTime.UtcNow));
        ...
        public bool TryDequeue(string topic, out MqttV2RawMessage message)
        {
            message = null;
            if (string.IsNullOrWhiteSpace(topic))
                return false;
        ...
        public async Task<MqttV2RawMessage> DequeueAsync(string topic, CancellationToken cancellationToken = default)
        {
            if (string.IsNullOrWhiteSpace(topic))
                throw new ArgumentException("topic is required.", nameof(topic));
            ...
            while (true)
            {
                ...
                catch (ObjectDisposedException)
                {
                    ...
                    throw new OperationCanceledException("Raw inbox was cleared while waiting for a message.", cancellationToken);
                }
            }
        }
