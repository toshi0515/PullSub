...
namespace UnityMqtt.V2.Core
{
    ...
    internal sealed class TypedDataRegistry
    {
        ...
        {
            private readonly IMqttV2PayloadCodec<T> _codec;
            private readonly TypedTopicCache<T> _cache;

            public TopicEntry(TypedTopicCache<T> cache, IMqttV2PayloadCodec<T> codec)
                : base(typeof(T), cache)
            {
                _cache = cache;
                _codec = codec;
            ...
            {
                return newCodec is IMqttV2PayloadCodec<T> codecAsT
                    && EqualityComparer<IMqttV2PayloadCodec<T>>.Default.Equals(_codec, codecAsT);
            }
        }
        ...
        public bool Register<T>(string topic, IMqttV2PayloadCodec<T> codec, out TypedTopicCache<T> cache)
        {
            lock (_gate)
            {
